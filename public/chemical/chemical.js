// Chemical.js client (vendored from the `chemicaljs` package, MIT).
// Patched for static hosting: no Chemical express server, so the templated
// constants below are inlined and the service worker path points at the
// app's existing combined proxy worker.
const demoMode = false;
const uvEnabled = true;
const rammerheadEnabled = false;
const scramjetEnabled = false;
const uvRandomPath = "uv";
const defaultService = "uv";
const chemicalOptions = (typeof window !== "undefined" && window.__chemicalOptions) || {};
const currentScript = document.currentScript || { dataset: chemicalOptions };
...
async function registerSW() {
  if ("serviceWorker" in navigator) {
    await navigator.serviceWorker.register("/proxy-sw.js", { scope: "/", updateViaCache: "none" });
  } else {
